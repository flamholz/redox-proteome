...
# We "fix" the growth rate by setting a maximum lambda value.
# the optimizer will always try to reach this value if possible.
...
phi_df.to_csv('../output/fix_lambda.csv', index=False)
...
for lam_val in lambdas:
    # Make fresh parameters with a new max_lambda_hr
    params = GrowthRateOptParams(min_phi_O=0.4, max_lambda_hr=lam_val, max_phi_H=0,
                                 maintenance_cost=maint, **DEFAULT_OPT_VALS)
...
phi_df['expected_Jana'] = phi_df['anabolism_gamma']*phi_df['anabolism_phi']
phi_df['expected_lambda'] = MW_C_ATOM*3600*phi_df['expected_Jana']
phi_df.to_csv('../output/fix_lambda_maint.csv', index=False)
...
phi_df = pd.DataFrame(results)
phi_df.to_csv('../output/fix_phi_red.csv', index=False)
...
S6_sensitivity_df.to_csv('../output/fix_ZCB.csv', index=False)

# Test variable carbon source redox
# TODO: rename ZCred to ZCred throughout
...
zcred_sensitivity_df.to_csv('../output/fix_ZCred.csv', index=False)


print('Sweeping pairs of (Z_C,red, S5) values...')
ZCreds = np.arange(-3, 3.01, 0.05)

# Default S5 = 0.3 ATP/C in anabolism 
# ~5 ATP/amino acid from the translation machinery gives 1 ATP/C
# But some of the biosynthetisis pathways yield ATP via oxidation.
# So 0.3 is a lower bound, realistically. The Cossetto preprint (2024)
# gives 10 ATP/C from meta-analysis of culture data. 
S5vals = np.concatenate([np.arange(0.1, 2.0, 0.05),
...
zcred_sensitivity_var_s5.to_csv('../output/fix_ZCred_S5.csv', index=False)
...
zcred_sensitivity_var_s4.to_csv('../output/fix_ZCred_S4.csv', index=False)
...
zcred_sensitivity_var_s3.to_csv('../output/fix_ZCred_S3.csv', index=False)
...
zcred_sensitivity_var_zcb.to_csv('../output/fix_ZCred_ZCB.csv', index=False)
...
zcred_sensitivity_var_zcb = pd.DataFrame(results)
zcred_sensitivity_var_zcb.to_csv('../output/fix_ZCred_ZCB_max_C_uptake.csv', index=False)
...
auto_gamma_df.to_csv('../output/auto_sampling.csv', index=False)

# Why did I call this "ext"?
auto_gamma_ext_C_df = pd.DataFrame(auto_ext_C_results)
auto_gamma_ext_C_df['model'] = 'autotrophy_ext_C'
auto_gamma_ext_C_df.to_csv('../output/auto_sampling_ext_C.csv', index=False)

gamma_df = pd.DataFrame(results)
gamma_df['model'] = 'heterotrophy'
gamma_df.to_csv('../output/heterotrophy_sampling.csv', index=False)
...
zcred_auto_df.to_csv('../output/autotrophy_fix_ZCred.csv', index=False)
...
phi_O_sensitivity_df.to_csv('../output/fix_phi_O.csv', index=False)
...
g_ana_sensitivity_df.to_csv('../output/fix_g_ana.csv', index=False)
...
g_red_sensitivity_df.to_csv('../output/fix_g_red.csv', index=False)
